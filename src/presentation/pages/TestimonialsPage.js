            ...
            ${REVIEWS.map(r => `
              <div class="card glass-surface-card" style="padding:clamp(22px, 3vw, 32px);display:flex;flex-direction:column;border-radius:16px;">
                ...
                <div style="border-top:1px solid rgba(201,162,75,0.2);padding-top:16px;display:flex;align-items:center;justify-content:space-between;">
                  <div>
                    <strong style="display:block;color:var(--gold-soft);font-size:1rem;font-family:var(--serif);">${r.author}</strong>
                    <span style="font-size:0.84rem;color:rgba(248,245,238,0.65);">${r.role} · ${r.city}</span>
                  </div>
                  <span style="font-size:0.8rem;color:rgba(248,245,238,0.5);">${r.date}</span>
                </div>
              </div>
            `).join('')}
          </div>
